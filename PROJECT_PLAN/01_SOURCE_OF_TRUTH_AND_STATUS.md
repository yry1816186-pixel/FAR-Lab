...
| `far verify-golden` | `IMPLEMENTED_VERIFIED` | `src/cli/commands/verify_golden.ts`：14 Golden Vectors 经真实 R0-R9 内核裁决（GV-01..GV-14）；`--backend node\|python\|browser` cross-lang 对拍；`--all` 全跑。GV 落地计数以 `far status --json` 为准 |
| `far fec compile` / `far fec freeze` | `IMPLEMENTED_VERIFIED` | `src/cli/commands/fec.ts`：FEC V2 编译（10 项检查·`compileFec`）+ `fecHash` 重算比对（`computeFecHash`·禁手填 RR-1）；示例契约 `examples/fec/sample_fec_contract.json`（`far fec compile --claim` 产出） |
| `far fsm advance` | `IMPLEMENTED_VERIFIED` | `src/cli/commands/fsm.ts`（P2-2）：9-state CLI 协议 FSM 推进（`src/cli/state_machine.ts`·CLAIM_CANDIDATE→…→VERIFIED）+ per-stage `stageReceipt` 哈希链（`computeStageReceipt`）；非法跳转 → `PROTOCOL_DEVIATION_CRITICAL` |
| `far demo` | `IMPLEMENTED_VERIFIED` | `src/cli/commands/demo.ts`：一键演示（offline·无需凭据）—— ① 14 GVs 经真实内核 ② demo chain（C-ASTRO-0001·机器裁决 UNTESTED·legacy 路径不注入统计→R6 不触发）③ hero pipeline（C-MMLU-A-0001·CONFIRMED via R7·真实 `oneSampleZTest` p/effectSize → ASK-9 密封降级 INCONCLUSIVE） |
| REST API server（`far api`） | `IMPLEMENTED_VERIFIED` | `src/api/server.ts` + `routes/`：Fastify HTTP API `/api/v1/verdict`/`/evidence` + `/health`/`/ready` 探针 + JWT/CORS/rate-limit；`far api` 启动器（默认 offline demo·种子 C-ASTRO 裁决·作为 frontend 网关）；OpenAPI schema 暴露于 `/documentation/json` |
| Web dashboard（`frontend/`） | `IMPLEMENTED_VERIFIED` | React + Vite 仪表盘 9 页（Overview/Viz/Integrity/Leaderboard/HonestyWall/Ablation/Report/DemoMode/About）·d3/reactflow 可视化·连 `localhost:3000` API；生产构建通过（`vite.config.ts` `build.target='es2022'`）；`far api` + `npm run dev` 全栈可运行。离线现场演示流程仍 `DESIGN_LOCKED` |
| `far ask "<q>" [--mode] [--json] [--export]` | `IMPLEMENTED_VERIFIED` | `src/cli/commands/ask.ts`：6-stage FSM 一次性 run + ASK-9 降级密封（复用 `executeLoop` + `sealProofEnvelope`）；`--export` 产出可验证 `.far-proof`（`far verify` 重算 clean）。默认 offline fixture（诚实标注非真实裁决） |
| `far stream "<q>"` | `IMPLEMENTED_VERIFIED` | `src/cli/commands/stream.ts`：同 ask 但实时流式打印每阶段（`runAgentLoop.onArtifact` 回调·真流非回放） |
| `far repl` | `IMPLEMENTED_VERIFIED` | `src/cli/commands/repl.ts`：交互式 REPL（readline·提问/`:fork`/`:history`/`:quit`·串行队列处理多行） |
| `far replay --db\|--bundle <p>` | `IMPLEMENTED_VERIFIED` | `src/cli/commands/replay.ts`：重放证据链（时光机·`call_records` 逐条·`verifyChainHead` hash 链重算验证） |
| `far court "<claim>" [--models]` | `IMPLEMENTED_VERIFIED` | `src/cli/commands/court.ts`：跨模型法庭（多 modelId 离线回放 + `ReliabilityCertificate`）；offline 下必然 unanimous（fixture 固定·诚实标注） |
