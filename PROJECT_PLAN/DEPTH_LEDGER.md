...
**当前态**：§C 全部接线行 = `WIRED_RED`。

`WIRED_RED` 表示接线代码与端到端测试均已落盘，且 `scripts/depth_gate.mjs`
证实符号级生产调用方（AST CallExpression）真实存在。`WIRED_GREEN`（唯一完成态）
须由 `scripts/depth_evidence.mjs` 在 CI 中以 base-FAIL / head-PASS 双跑物证写回——
agent 不得手填（见 §B、§D CHECK-L2）。

**为何停留 WIRED_RED**：核心 P0–P3 与 FUSION-OS-1..14 的接线均已完成并验证
（typecheck / lint / 全量测试 / depth_gate / 四扫描全绿），但物证升级需要：

1. maintainer 在 GitHub 配置分支保护、Actions 写入权限、CODEOWNERS（见
   `CONTRIBUTING.md`「Repository Configuration」一节）；
2. keystone bot 在真实 CI checkout 上对每行 proof_test 双跑，base 须 FAIL、
   head 须 PASS。

接线以批量提交方式落地，不存在「测试存在但失败」的中间基线，故部分行的
base-FAIL 无法经现有 git 历史自动证明——这类行须 maintainer 选更早基线，或由
CODEOWNERS 手动背书。
