...
## 五、本 Wave 量化野心（北极星映射）
- 直接负责：run-reliability（自动检测+恢复：注入失败 100% 在一个轮询周期内可恢复；stretch = 20-run soak 零冻结）；run-wall-clock（若并行化：p50 <=4.5min 且结果等价性证明）。
- 准入线：无量化痛点即 KEEP（拒绝为炫技加复杂度）；有痛点则恢复时间/成功率量化 >=50% 改善。
- 反回归：并行化不得改变任何确定性测试结果（同 seed 同输出）。
