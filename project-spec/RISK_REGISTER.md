...
| R-19 | exploration CodeAct static gate is bypassable (alias/split-chain/getattr; static bans are guardrails, not a jail) | agent-drafted code reaches researcher-privilege OS surface | CLOSED 2026-09-02: fail-closed Docker Linux OCI boundary (non-root UID/GID, network none, read-only rootfs, no capabilities, NNP+seccomp, cgroup/private-namespace limits, pre-start inspect + in-container attestation, forced cleanup) verified on the real caller, failure-injection and adversarial paths LOCALLY, and on canonical main 0da4c68 HOSTED ubuntu (run 33583828122: image sha256:fedefb6e…, Linux /proc attach-client cleanup checked=4 remaining=0; B-FA-SEC-01-HOSTED-OCI RESOLVED, FA-SEC-01 PASS). The production `runExploration` runner and `wireResearchTools` own that factory and expose no caller-supplied sidecar; deterministic tests replace only the module boundary inside an isolated Vitest worker. Stronger profiles (rootless/gVisor/VM/Windows-native) stay D-SEC-01 declaration-triggered tiers. |
| R-20 | no process-boundary network egress allowlist (sources layer guarded 2026-08-30; providers/MCP/custom routes not) | injected tool use or escape exfiltrates to arbitrary hosts | CLOSED at FA-SEC-04 scope 2026-09-02: single policy owner src/shared/destination-guard.ts enforced at every audited egress chokepoint (sources re-export, providers transport fail-closed pre-dispatch, MCP HTTP connect, configurable Zotero base, OpenML upstream-returned download URL), 9 regressions; disclosed residual: DNS names that privately resolve (rebinding) are invisible to a static guard — network-level enforcement stays with the sandbox/OS profile |
