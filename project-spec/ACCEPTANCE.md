...
| ACC-40 | Lineage storage single authority: lineage_edges + event_tags (migration v5) with adjacency + read-time traversal; edges are written at object-creation time (live writers, not only backfill) and the API/CLI projection reads from the table; vocabulary owned by one domain module | tested |
| ACC-41 | Exploratory CodeAct execution: TS static gate verdict is enforced before spawn; the production caller then uses a dedicated Linux OCI container with an exact environment allowlist, no network, read-only rootfs, non-root UID/GID, no capabilities, NNP+seccomp, private namespaces, resource limits, pre-start inspection, in-container attestation and forced cleanup. Production capability wiring owns the Docker factory and does not accept a caller-supplied sidecar. Missing/invalid Docker isolation fails closed with no host-side fallback. Runtime failures remain visible candidate findings and promotion stays behind deterministic confirmatory gates | tested |
| ACC-42 | ScientificProblemModel + MethodSelection are formed at scope inside closed schemas (LLM proposes, deterministic code owns ids/enums/validation); the selected family names a real validator; disclosure reaches hypotheses/plan payloads, the paper Methods section and the map; execute-leg routing derives deterministically from method selection (rejected families skip their legs with an audit note) | live_verified |
| ACC-43 | Numerical execution leg: preregistered FEM (uniform + adaptive AFEM) and theory-identity specs execute in the pinned Python sidecar; verdicts derive mechanically from preregistered thresholds (convergence order / identity residual); numerical evidence binds into the reproducibility bundle and re-verifies | live_verified |
| ACC-44 | Scientific data plane: NetCDF acquisition with immutable raw and read-back sha256 (TOCTOU fence), QC verdicts at record time, derived versions with processing lineage; registered datasets auto-serialize into experiment specs with paths resolved by deterministic code (never prompts); datasetEvidence resolvability is a bundle verify check | live_verified |
| ACC-45 | Protocol layer for work the software cannot execute: ProtocolSpec preregistration (materials/instruments/arms/randomization/QC/human-confirmation nodes/ethics gate fail-closed), append-only human-attested ledger, plan-derived draft with disclosed adjustments, protocol evidence + disclosure-laundering guard in export/verify; real human execution is disclosed as user-owned, never simulated | tested |
| ACC-46 | AOSSA proof scenarios: A (2D Poisson FEM closed loop) and B (literature + NetCDF ML) completed end-to-end on the live route with independently verified bundles; direct-LLM and coding-agent baselines scored on predeclared structural metrics; external-product baselines and scenario C's human leg stay disclosed blockers, not gaps papered over | live_verified |
