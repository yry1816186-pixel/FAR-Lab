...
| `0010_proof_envelopes_v2.sql` | 10 | **04 §2.1（task #9）** proof_envelopes_v2（ProofEnvelope V2·16 字段 + proof_hash self-excluding + anti-theater trigger）⚠ **forward-schema（无生产写入器）**：V2 表建表+完整 triggers 但无 production INSERT（零 v2 repository）；V1 表 `proof_envelopes`（0004）为活跃持久化路径（`sealer.ts`）；V2 逻辑（sealer/validator/proof_hash）服务 verify + 跨语言 proofHash 路径。V2 持久化接线 vs 弃用表 = DESIGN 裁决（DEBT-06 文档化半已完成，策略决策待 scoped DESIGN）| `04 §2.1/§2.4` + `APPENDIX_C §2` + task #9 |
