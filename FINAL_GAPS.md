...
1. **平台事实链基本闭合**：canonical main `0da4c68` run 33583828122 全绿（三 OS verify、chromium full 26/26、firefox/webkit smoke、exploration-sandbox、CodeQL×4）——FA-PLT-01/02 已闭。仍开放：hosted attested release-pack/OIDC 签名/GitHub Release（FA-PLT-03/FA-SEC-09/10）与 macOS 全量+webkit 非 smoke 深度。ACC-25 "real Linux target" 措辞已闭（FA-REM-01）。
...
| ID | 缺口 | owner | 状态 |
|---|---|---|---|
| FA-PLT-02 | canonical CI：verify 绿、web-e2e loaded-home CLS 红 | self | **已闭（2026-09-02：canonical main 0da4c68 run 33583828122 全绿——三 OS verify+chromium full e2e 26/26；87a1f3f CLS 修复经 PR #137 集成后 hosted 核验通过）** |
...
仍开放：FA-W0-05 五表 SWEEP 已完成 3/5 表（product_specs_docs 42/42 + governance_assets 61/64（余 3 件为账本活文件本身，终局冻结时裁决）+ delivery_operations 127/127，2026-09-02 主代理+并行代理交叉裁决，17 处真实缺陷随审随修；runtime 389/tests 340 审查代理已分派本窗）· FA-W0-06 浅断言 0/188 待逐项裁决 · ~~FA-HAR-01 per-tool 超时缝~~ **已闭（2026-09-02，e944c57：step 剩余预算竞速+挂死工具 step_timeout 终局+回归）** · ~~FA-HAR-02 预算/pacing 默认化~~ **已闭（2026-09-01，9fb2f29 前序 6b8e14d：pacing 默认 600ms/每 provider，env 可关；预算默认 120s 先前已在）** · FA-HAR-03 failover live 实证 · FA-HAR-04 72h soak · **新增登记（2026-09-02，PR #139 run 33588116282）：hosted chromium map CLS 间歇竞态——同树同套件双跑 0.3011 vs 0.0298（7 claims/0 hyps 稀疏形状），spineLoaded 门（87a1f3f）未闭净的残差；main 0da4c68 曾 26/26 绿，属间歇非回归；需专门归因（evidence completion 与 /science settle 的时序窗口）后加门或重校预算（取证代理已分派本窗）** · FA-DAT-01 数据面流式化（实现已落：流式 put/哈希/CSV 行分析/列视图 split/netcdf 去帽/http 体积帽；等价回归 14 测试过 + typecheck/build 绿；容量基准脚本 scripts/capacity-benchmark.mjs 已落，1GB CSV/500MB NetCDF 全量跑移交下一窗口——本收尾窗用户指令免重跑，仍 PARTIAL） · ~~FA-SCI-05 ODE 腿实现~~ **已闭（2026-09-01，9fb2f29：域+sidecar op+executor+起草器+execute 级联+8 测试含真实 sidecar 解析解对照）** · FA-SCI-06 结构化输出复测 · FA-SCI-07 AstaBench/MLR 接入 · FA-SCI-04 执行 · ~~FA-REM-02 远程 cell 级去重~~ **已闭（2026-09-02，ce87d9c：共享 cell-dedup 模块接入 remote——同设备重试 0 重训回放缓存细胞/跨设备诚实重训/失败路径 /tmp 清理；等价回归 5 例+真 docker e2e 过）** · FA-PRF-02 后端 perf gate（已落地，hosted 首跑随 main CI）· FA-PRF-03 混沌矩阵补 4 项 · ~~FA-SEC-08 fence crypto-random~~ **已闭（2026-09-02，08bcaf4：randomBytes(6) 48bit+熵回归）** · FA-SEC-09 hosted SBOM/SAST/audit 结果 · FA-SEC-11 威胁登记（R-19/20 已闭；R-23/24 本日补登；R-21 条目闭待 FA-SEC-09/10 能力闭） · ~~FA-PLT-01 windows/firefox hosted 绿证~~ **已闭（2026-09-01：495e53c 车道+main 全绿，windows verify/firefox e2e/desktop 三平台+release-pack 全过）** · FA-PLT-03 hosted attested release-pack/GitHub Release · FA-HCI-01..03（兄弟车道）· FA-EVAL-02 user-study 执行（协议/材料/仪表化已开发完毕，BLOCKED_EXTERNAL 被试）· FA-EVAL-04 ~~已闭（2026-09-02：FRONTIER_STATUS lastCIgreenSHA+scienceMetrics+刷新规则）~~。

## P2 队列（architecture/product/performance/frontier 摘要）

FA-HAR-05 steer 接线 · FA-HAR-06 记忆管理面 + 7d 验证 · ~~FA-DAT-03 静态加密决策~~ **已闭（2026-09-02：SECURITY.md 显式 non-goal+FDE 责任边界+多用户重开触发器）** · FA-DAT-04 FTS 增量 · FA-DAT-05 verify 空转收紧 · FA-REM-04 Slurm 网关 · FA-SEC-10 hosted OIDC 签名+updater · ~~FA-SEC-12 web 解压上限~~ **已闭（2026-09-02，0b26d06：jszip 面三限预算+mammoth/pdfjs 残差披露）** · FA-PLT-05 desktop 签名/更新/卸载 · FA-SCI-08 外部产品对照 · FA-EVAL-01/03 MLR 全量/prompt 工程化 · ~~FA-HCI-04 EN 泄漏残项~~ **已闭（2026-09-02，e5052a6：promoteToFeedback 三键 dict 化）**。FA-PLT-04/06 已闭合为 PASS。
