...
| **FI-5 Bench-125** | 125 题全 fixture + 套件根可验 + Resolution Curve | 复用已落地 `suiteIntegrityRoot` + SuiteVerifier；评委浏览器独立重算 125 题套件根 | Resolution Curve 须过 R5 三审查门（反自我指涉 theater 质量门）；不过门则曲线本身不做（Bench-125 仍满血） | `ROADMAP`（8 seed 已 `IMPLEMENTED_VERIFIED`：A4/A16/B7/C3/E2/G5 + P1 物理 + M2 医学·7 领域·5 verdict 全覆盖·套件根可验；8→125 仍 ROADMAP 需大量领域内容） |
| **FI-6 WASM + 流式** | WASM 三向字节相等 + 流式 hash-link + perf 基准 | TS ≡ Python ≡ WASM 三向 golden；canonicalHash µs 级基准进 CI | 若 `1e-7` 鸿沟在 WASM 编译产物重现，诚实降级双语言 + JS（技术约束，非时间妥协） | `BLOCKED_on_V3`（2026-07-08 实证：AssemblyScript 与 Rust/ryu 的 f64.toString 均与 V8 不等——AS 整数浮点加 ".0"（1.0→"1.0" vs V8 "1"）·更糟 0.1+0.2 给出不同 digits（AS "0.30000000000000007" vs V8 "0.30000000000000004"）。V8 Number.toString 独特算法须 reimplement = canonical float serializer V3 RESEARCH 同源。维持双语言 node/python + browser(V8) 三向字节相等） |
