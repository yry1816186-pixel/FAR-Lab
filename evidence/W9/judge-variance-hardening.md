...
## Full-scope live measurement (2026-08-22, D-059 — supersedes the 2/5 numbers above)

After generating the 3 missing runs on glm-5.3 (generation itself on the production Anthropic-wire zai route), the FULL 5/5 measurement returned **worstTaskSwing 0.19 — target 0.15 NOT met**. Per task: arg/egfr/cdiff/crispr swing exactly **0** (deterministic layer + fixed GT + 3-pass median = zero variance, as constructed); the entire residual is ONE borderline-heavy task (crc, 9–13 adjudicated pairs/pass) where 3-vote majorities flip between passes (0.333/0.143/0.235). The earlier 2/5-scope 0.061 (D-058) had not included this task and is superseded. This MEASURES the structural ceiling predicted offline (borderline band carries the F1 information; semantic adjudication is the load-bearing layer). Next levers (recorded, not executed — no cherry-picked re-runs): adjudication votes 3→5; anchored per-pair adjudication prompt; borderline-band narrowing via stronger lexical features.
