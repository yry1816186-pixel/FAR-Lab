...
const NODE_KIND_COLOR_MAP: Record<string, { fill: string; stroke: string; label: string }> = {
  hypothesis: { fill: 'hsl(217.2, 91.2%, 59.8%)', stroke: 'hsl(217.2, 91.2%, 45%)', label: '蓝' },
  evidence:   { fill: 'hsl(142.1, 70.6%, 45.3%)', stroke: 'hsl(142.1, 70.6%, 32%)', label: '绿' },
  method:     { fill: 'hsl(262.1, 83.3%, 57.8%)', stroke: 'hsl(262.1, 83.3%, 43%)', label: '紫' },
  plan:       { fill: 'hsl(32.1, 94.6%, 43.7%)', stroke: 'hsl(32.1, 94.6%, 33%)', label: '橙' },
  feedback:   { fill: 'hsl(47.9, 95.8%, 53.1%)', stroke: 'hsl(47.9, 95.8%, 40%)', label: '黄' },
  root:       { fill: 'hsl(215.4, 16.3%, 46.9%)', stroke: 'hsl(215.4, 16.3%, 35%)', label: '灰' },
};
...
const NO_SOURCE_ANCHOR = '(节点 DTO 不含 sourceAnchor)';
const NO_HASH = '(节点 DTO 不含 hash)';
...
function ForceGraphPanel({ subtree, onSelectNode }: ForceGraphPanelProps) {
  ...
  const tooltipContent = hoveredNode !== null ? (
    <div className="space-y-1">
      ...
      <div className="text-xs text-muted-foreground">证据ID: {hoveredNode.evidenceId}</div>
      ...
      <div className="text-xs text-muted-foreground">
        指标值: {hoveredNode.metricValue !== null ? hoveredNode.metricValue.toFixed(4) : '—'}
      </div>
      <div className="text-xs text-muted-foreground">冲突证据: {hoveredNode.conflictingEvidenceCount}</div>
  ...
  return (
    <Card className="w-80 shrink-0 self-start" data-testid="node-detail-sidebar">
      ...
      <CardContent className="space-y-3">
        <DetailRow label="节点 ID" value={node.nodeId} mono />
        <DetailRow label="证据 ID" value={node.evidenceId} mono />
        <div className="flex items-center gap-2">
          <span className="text-sm text-muted-foreground w-20 shrink-0">节点类型</span>
          <Badge variant={NODE_KIND_BADGE_VARIANT[node.nodeKind] ?? 'outline'}>
            {NODE_KIND_LABEL[node.nodeKind] ?? node.nodeKind}
          </Badge>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-sm text-muted-foreground w-20 shrink-0">判定</span>
        ...
        {node.metricValue !== null && (
          <DetailRow label="指标值" value={String(node.metricValue)} />
        )}
        {node.conflictingEvidenceCount > 0 && (
          <DetailRow label="冲突证据数" value={String(node.conflictingEvidenceCount)} />
        )}
        {node.scopeSlipText !== null && (
          <DetailRow label="范围滑移" value={node.scopeSlipText} />
        )}
        {node.untestedReason !== null && (
          <DetailRow label="未测试原因" value={node.untestedReason} />
        )}
        {node.parentNodeId !== null && (
          <DetailRow label="父节点" value={node.parentNodeId} mono />
        )}
        <DetailRow label="sourceAnchor" value={NO_SOURCE_ANCHOR} />
        <DetailRow label="hash" value={NO_HASH} />
        <DetailRow label="创建时间" value={node.createdAt} mono />
      </CardContent>
    </Card>
  );
}
...
export default function VizPage() {
  ...
  return (
    <div className="space-y-6" data-testid="viz-page">
      {/* 页头 */}
      <header>
        <h1 className="text-3xl font-bold tracking-tight">证据链可视化</h1>
        <p className="mt-1 text-muted-foreground">
          输入证据链头哈希（headHash），以力导向 DAG 浏览证据节点与边。节点颜色按 5 值裁决映射，悬停查看详情。
        </p>
      </header>
        ...
        <CardContent className="pt-6">
          <div className="flex gap-2" data-testid="search-bar">
            ...
            <input
              ...
              placeholder="输入 headHash（64 位 hex）"
      ...
      {isEmpty && (
        <Alert data-testid="viz-empty">
          ...
          <AlertTitle>空证据链</AlertTitle>
          <AlertDescription>
            该 headHash 对应的证据链不含任何图节点。可能原因：hash 不存在、证据链尚未构建、或数据已被清理。
          </AlertDescription>
        </Alert>
      )}
      ...
      {subtree !== null && subtree.nodes.length > 0 && (
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>节点颜色按 5 值裁决映射（主着色），节点类型与边样式辅助区分</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <span className="text-sm font-medium">裁决着色（主）</span>
              <div className="mt-2">
                <VerdictLegend />
              </div>
            </div>
            <div>
              <span className="text-sm font-medium">节点类型（辅助）</span>
              <div className="mt-2">
                <NodeLegend />
              </div>
            </div>
            <div>
              <span className="text-sm font-medium">边类型</span>
              <div className="mt-2">
                <EdgeLegend />
              </div>
            </div>
      ...
      {hasSearched && !isLoading && !isError && subtree === null && (
        <Alert data-testid="viz-no-subtree">
          <AlertTriangle className="h-4 w-4" />
          <AlertTitle>无法解析证据链图数据</AlertTitle>
          <AlertDescription>
            API 返回的 graphSubtree 不符合预期形状（缺少 rootId / nodes / edges）。请检查后端 API 版本是否匹配。
          </AlertDescription>
        </Alert>
      )}
      ...
      {!hasSearched && (
        <div className="flex flex-col items-center justify-center py-20 text-center" data-testid="viz-initial">
          ...
          <h2 className="mt-4 text-lg font-semibold">输入 headHash 开始探索</h2>
          <p className="mt-1 text-sm text-muted-foreground max-w-md">
            证据链可视化以力导向 DAG 展示证据节点间的支持、反驳、派生、测试与迭代关系。每个节点代表一条可审计的判定记录，颜色按 5 值裁决映射，悬停可查看证据详情。
          </p>
        </div>
      )}
    </div>
