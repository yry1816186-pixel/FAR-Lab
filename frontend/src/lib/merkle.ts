/**
 * merkle —— 浏览器侧 Merkle 包含证明独立重算（Live Reproof + Tamper Theatre 核心）。
 *
 ...
 * 生产浏览器（Chrome/Firefox/Safari/Edge）100% 提供 crypto.subtle；vitest jsdom 环境由
 * test-setup.ts 注入 Node WebCrypto 补齐 subtle。
 *
 ...
 *   产出**字节相同**的摘要（由 __tests__/IntegrityPage.test.tsx 用 GOLDEN_INTEGRITY_* 断言）。
 *
...
/** 浏览器侧包含证明形状（镜像后端 MerkleInclusionProof / 前端 IntegrityProofDto 的证明子集）。 */
...
function assertHex64(value: unknown, context: string): void {
  if (typeof value !== 'string' || !HEX64.test(value)) {
    throw new Error(`${context}: expected 64-char lowercase hex SHA-256, got "${value}"`);
  }
}
