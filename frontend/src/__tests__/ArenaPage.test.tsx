...
describe('ArenaPage', () => {
  ...
  it('渲染 ROBUST 裁决 + 3 refuter 记分板（全 held）', async () => {
    ...
    expect(screen.getByText('对抗科学竞技场')).toBeInTheDocument();
    expect(screen.getByText('01KWZTESTARENARST456')).toBeInTheDocument();

    // 3 refuter 全 held（多匹配）
    expect(screen.getAllByText(/held（抗住）/).length).toBe(3);
    ...
    expect(screen.getByText('诚实声明')).toBeInTheDocument();
  });
  ...
  it('渲染失败态（fetch 500）', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('竞技场会话失败')).toBeInTheDocument();
    });
  });
});
