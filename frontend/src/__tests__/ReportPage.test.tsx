...
describe('ReportPage', () => {
  ...
  it('渲染页面容器与标题', () => {
    ...
    expect(screen.getByRole('heading', { name: '研究报告', level: 1 })).toBeInTheDocument();
  });
  ...
  it('未输入 runId 时显示空闲提示', () => {
    ...
    expect(screen.getByTestId('report-idle')).toHaveTextContent('输入 runId');
  });
  ...
  it('报告 API 返回非 2xx 时展示错误信息', async () => {
    ...
    expect(screen.getByTestId('report-error')).toHaveTextContent('加载报告失败');
  });
  ...
  it('报告内容为空字符串时显示空报告提示', async () => {
    ...
    expect(screen.getByTestId('report-empty')).toHaveTextContent('报告内容为空');
  });
  ...
  it('历史为空时显示“暂无查看历史”', () => {
    ...
    expect(screen.getByTestId('report-history-empty')).toHaveTextContent('暂无查看历史');
  });
  ...
  it('判词数据为空时显示“暂无判词数据”', async () => {
    ...
    expect(screen.getByTestId('report-verdict-empty')).toHaveTextContent('暂无判词数据');
  });
