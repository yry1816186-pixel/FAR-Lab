...
describe('IntegrityPage', () => {
  ...
  it('渲染页面容器与标题', () => {
    ...
    expect(
      screen.getByRole('heading', { name: '完整性信任根', level: 1 }),
    ).toBeInTheDocument();
  });
  ...
  it('Live Reproof：拉取证明 + 浏览器 Web Crypto 独立重算通过（ok=true）', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('verify-result')).toHaveTextContent('验证通过');
    });
  });
  ...
  it('Tamper Theatre：篡改叶 → 根不匹配 + 篡改已检测；恢复 → 再次通过', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('verify-result')).toHaveTextContent('根不匹配');
    });
  ...
  it('CrossLang：默认 golden 叶对 → 浏览器 combine 与 Node/Python golden 字节相等', async () => {
    ...
    expect(screen.getByTestId('golden-compare')).toHaveTextContent('字节相等');
  });
  ...
  it('HonestyWall：诚实声明条目渲染（已知边界如实标注）', () => {
    ...
    expect(screen.getByTestId('honesty-wall')).toHaveTextContent('裁决重入尚未实现');
    expect(screen.getByTestId('honesty-wall')).toHaveTextContent('跨语言数值域仍有已知鸿沟');
  });
