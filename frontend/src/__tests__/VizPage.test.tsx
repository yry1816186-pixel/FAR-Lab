...
describe('VizPage', () => {
  it('渲染初始空态：输入框 + 查询按钮 + 引导文案', () => {
    ...
    expect(screen.getByText('输入 headHash 开始探索')).toBeInTheDocument();
  });
  ...
  it('渲染图例（裁决着色 + 节点类型 + 边类型）', async () => {
    ...
    expect(screen.getByText('已驳斥')).toBeInTheDocument();
    // 节点类型图例
  ...
  it('节点圆包含 SVG title 悬停提示', async () => {
    ...
    expect(firstTitle).toContain('证据ID');
    expect(firstTitle).toContain('裁决');
    expect(firstTitle).toContain('指标值');
  });
  ...
  it('点击节点打开侧栏详情', async () => {
    ...
    expect(screen.getByTestId('node-detail-sidebar')).toHaveTextContent('节点详情');
  });
  ...
  it('关闭侧栏按钮隐藏详情', async () => {
    ...
    const closeBtn = screen.getByRole('button', { name: '关闭侧栏' });
  ...
  it('节点详情侧栏使用 VerdictBadge 展示判定', async () => {
    ...
    await waitFor(() => {
      ...
      expect(sidebar).toHaveTextContent('证据');
      expect(sidebar).toHaveTextContent('已确认');
