...
describe('DemoModePage', () => {
  ...
  it('demo-03: 页头诚实声明当前为功能导览子集（非 spec 16 现场演示）', () => {
    ...
    expect(note.textContent).toContain('功能导览子集');
  ...
  it('从第 1 幕开始（scene-card-0）', () => {
    ...
    expect(screen.getByText('FAR 三大支柱')).toBeInTheDocument();
  });
  ...
  it('第 8 幕时下一幕按钮禁用', async () => {
    ...
    expect(screen.getByText('复现与审计')).toBeInTheDocument();
  ...
  it('显示诚实标注弹窗', async () => {
    ...
    expect(screen.getByText('诚实标注')).toBeInTheDocument();

    // 关闭弹窗
    await user.click(screen.getByLabelText('关闭诚实标注'));
    expect(screen.queryByTestId('honesty-popover')).toBeNull();
  });
