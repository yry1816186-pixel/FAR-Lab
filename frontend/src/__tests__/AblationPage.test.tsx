...
describe('AblationPage', () => {
  ...
  it('renders page container with header, input, and disabled run button', () => {
    ...
    expect(
      screen.getByRole('heading', { name: '消融实验', level: 1 }),
    ).toBeInTheDocument();
  ...
  it('displays verdict badges with correct Chinese labels in table', async () => {
    const user = userEvent.setup();
    ...
    expect(table.textContent).toContain('已确认');
    expect(table.textContent).toContain('已证伪');
    expect(table.textContent).toContain('不确定');
    expect(table.textContent).toContain('未测试');
  });
  ...
  it('shows termination reason badges in table', async () => {
    ...
    const table = screen.getByTestId('comparison-table');
    expect(table.textContent).toContain('收敛');
    expect(table.textContent).toContain('最大迭代');
    expect(table.textContent).toContain('错误终止');
    expect(table.textContent).toContain('Token 上限');
  });
  ...
  it('shows all-error fallback when every baseline fails', async () => {
    ...
    expect(screen.getByText('全部基线运行失败')).toBeInTheDocument();
  });
  ...
  it('渲染 HonestyWall：声明离线 fixture 相同 + 护城河不在 verdict', () => {
    renderWithQueryClient(<AblationPage />);
    const wall = screen.getByTestId('ablation-honesty-wall');
    expect(wall).toBeInTheDocument();
    expect(wall.textContent).toContain('相同的确定性 loopState');
    expect(wall.textContent).toContain('不在 verdict 数值');
  });

  it('渲染 CapabilityMatrix：4 方法 × 6 能力维度（testid 齐全）', () => {
    renderWithQueryClient(<AblationPage />);
  ...
  it('能力矩阵归属：far-chain 全部具备·random 全部不具备（真实定性·非编造数值）', () => {
    renderWithQueryClient(<AblationPage />);
    const dims = [
      'evidenceRetrieval',
      'structuredChain',
    ...
    for (const dim of dims) {
      ...
      expect(farCell.querySelector('[aria-label="具备"]')).not.toBeNull();
      const randCell = screen.getByTestId(`cap-${dim}-random`);
      expect(randCell.querySelector('[aria-label="不具备"]')).not.toBeNull();
    }
  });
});
