...
export function FalsifiabilityChart({ data }: FalsifiabilityChartProps) {
  ...
  useEffect(() => {
    ...
    g.selectAll('.fals-label')
      ...
      .attr('font-weight', '600')
      .text((d) =>
        hasFalsificationSpec(d.response) ? '有可证伪规格' : '无可证伪规格',
      );
