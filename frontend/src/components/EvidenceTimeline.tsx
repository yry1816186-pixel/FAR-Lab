      ...
      <div className="grid grid-cols-1 gap-1.5">
        ...
        {sourceAnchor.dashscopeRequestId !== undefined && sourceAnchor.dashscopeRequestId !== null && (
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-muted-foreground text-xs">请求 ID:</span>
        ...
        {sourceAnchor.dashscopeRequestId === null && (
          <div className="flex items-center gap-1.5">
            <Globe className="h-3.5 w-3.5 shrink-0 text-muted-foreground" aria-hidden="true" />
            <span className="text-muted-foreground text-xs">请求 ID:</span>
            <span className="text-xs text-verdict-inconclusive font-medium">凭证缺失 (null)</span>
          </div>
        )}
        {sourceAnchor.rawResponseHash !== undefined && (
          <div className="flex items-center gap-1.5">
            <Hash className="h-3.5 w-3.5 shrink-0 text-muted-foreground" aria-hidden="true" />
            <span className="text-muted-foreground text-xs">响应 Hash:</span>
    ...
    >
      ...
      <div className="flex flex-col items-center gap-1">
        ...
        <div className="w-full rounded bg-background px-2 py-1 border border-primary/30">
          <span className="text-xs text-muted-foreground">当前哈希 (currentHash)</span>
      ...
      {!isGenesis && (
        <p className="text-[11px] text-muted-foreground text-center">
          验证：sha256(prevHash ‖ payload) = currentHash。链式完整性保证证据未被篡改。
        </p>
      )}
    </div>
  );
...
function FalsificationSpecSummary({ item }: { item: HonestVerdictDto }) {
  ...
  return (
    <div className="rounded-md bg-muted/50 px-3 py-2 text-sm">
      <span className="text-muted-foreground">可证伪断言：</span>
  ...
  return (
    ...
    >
      ...
      <Card
        ...
        aria-label={`裁决 ${item.verdictId} - ${config.label}`}
      >
        ...
        <CardContent className="space-y-2">
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
            ...
            <span className="text-muted-foreground">冲突证据数</span>
          ...
          {item.decision === 'DEGRADED_SCOPE' && item.scopeSlipText !== null && (
            ...
            >
              <span className="font-semibold text-orange-800">范围降级说明：</span>
          ...
          {item.decision === 'UNTESTED' && item.untestedReason !== null && (
            ...
            >
              <span className="font-semibold text-gray-700">未测试原因：</span>
  ...
  return (
    <div className="space-y-0" data-testid="evidence-timeline">
      ...
      <div className="flex items-center gap-4 pb-2">
        ...
        <span className="text-xs text-muted-foreground">证据链末端</span>
      </div>
    </div>
  );
}
