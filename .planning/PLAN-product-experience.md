...
- **B2（已落地 2026-08-22，GUI 全验证 + 三视角批判修复）**：①IA：证据页重排（来源→主张→关系升首；检索透明降为折叠披露）；来源表补作者/期刊/DOI/OA 链接列，hash/parse 列降入「技术校验明细」折叠列表（parse!=ok 时主行保留内联警示）；claims 陈述为主标签（ID 退居行尾）；概览元信息降为技术披露（运行告警保持可见）；②全局搜索：`GET /api/v1/search?q=`（SQL LIKE+ESCAPE，question 经 runs 表 questionId 映射 run；测试 5 项含通配符转义/限长/405）；命令面板 ≥2 字符防抖 220ms 跨 run 搜研究/假设/主张，点击/Enter 导航（claim→证据页+闪光定位带重试与必消费语义）；③quick capture：`n` 键→欢迎页+输入框自动聚焦；④侧栏：「需注意」组默认折叠（过滤/选中自动展开），run ID 降为 tooltip；⑤键盘/ARIA 加固（批判修复）：面板焦点开/闭归还、Tab 模态圈禁、option 扁平化（组标签 presentation 行、无嵌套按钮）、索引钳制入按键处理器、h3 内嵌 button 合法模式、summary 对比度 AA、--fs-aux 笔误修正、DOI 格式守卫+OA https 守卫。**批判假阳性已验证驳回**：CJK 搜索（SQLite LIKE 字节级匹配正常，语料被搜字段全英文属数据事实）；被取代版本暴露（同 id INSERT OR REPLACE 只存当前版）。**记录待办**：搜索排序/片段高亮/总数 hasMore（B14 打磨）；oaUrl 检索侧从未填充（B3/B9 管线债）；假设生命周期 status 字段（B5 Fork/Promote/Reject 时引入）。门禁：vitest 661/663 + 双端 tsc/build 0 + secret-scan PASS。
- **B3（第一部分已落地 2026-08-22，真实 run 全程验证）**：①receipt 事件富化——模型调用带 provider/modelId/latencyMs（GUI 实证「模型调用 · glm-4.6 · 4270ms」）、检索带 family/query/httpStatus/resultCount（「检索 · crossref · 6 命中 — 查询词」）；②StageRecord.subtasks 真实总量填充：retrieve/verify_sources/build_evidence/critique_falsify 四阶段（电池 run 实测 14/14、12/12、8/8、逐假设 x/10）；③note 里程碑：query_plan_ready/document_extracted/hypothesis_critiqued（17 条实测，假设带陈述文本逐条浮现）；④ActivityFeed 富化渲染 + 里程碑行；⑤**SSE 事件流**：`GET /runs/:id/events/stream`（Node 原生零依赖，Last-Event-ID 续传，10min 生命周期+自动重连，ping 保活）+ useEventStream 客户端（失败自动回退轮询，SSE 健康时轮询降为 15s 安全网；连接建立 netstat 实证）；⑦版本对比：修订操作 before/after 全文并排展开（原生 details，网格双栏，v0/v1 标注）；⑧ExperimentsTab 全面 i18n 化（useResource 对齐 + t() 全覆盖，EN 下零中文）。**待办（B3 尾巴）**：⑥web 通知 + 多 run 感知条；版本对比展开与实验页 EN 的 GUI 视觉验证（IAB 会话退化阻塞交互，标记 ENV-LIMITED，B13 a11y 批次用新会话复验）。门禁：vitest 661/663 + 双端 tsc/build 0 + secret-scan PASS。
