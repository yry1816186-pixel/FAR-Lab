...
describe('retrieve stage', () => {
  it('SCIENCE lane: citation chase snowballs the pool — forward cites: + backward referenced_works, receipted, competing in the same RRF/cap', async () => {
    // Seed record carries an OpenAlex id + referenced_works, so it can seed both
    // chase directions. Pool stays far under the cap (no rerank, no LLM beyond plan).
    const seedRec = fakeRecord('Fixture Seed Work', '10.1000/chase.seed', {
      identifiers: [
        { kind: 'doi', value: '10.1000/chase.seed' },
        { kind: 'openalex', value: 'W100' },
      ],
      normalized: { DOI: '10.1000/chase.seed', referenced_works: ['W900'], fixture: true },
    });
    const forwardRec = fakeRecord('Fixture Forward Citing Work', '10.1000/chase.forward', {
      identifiers: [
        { kind: 'doi', value: '10.1000/chase.forward' },
        { kind: 'openalex', value: 'W700' },
      ],
    });
    const backwardRec = fakeRecord('Fixture Backward Cited Work', '10.1000/chase.backward', {
      identifiers: [
        { kind: 'doi', value: '10.1000/chase.backward' },
    ...
    const openalex = fakeAdapter('openalex', {
      search: byQuery({
        [PLAN.discovery[0] as string]: [seedRec],
        [PLAN.counter[0] as string]: [],
        [PLAN.counter[1] as string]: [],
        [PLAN.supporting[0] as string]: [],
      }),
    });
    // Extend the fake with the OPTIONAL filter API (only openalex implements it).
    (openalex as SourceAdapter & {
      searchFiltered?: (filter: string, opts?: { limit?: number }) => Promise<{ family: SourceFamily; query: string; httpStatus: number; records: RawSourceRecord[]; latencyMs: number }>;
    }).searchFiltered = async (filter) => {
      filteredCalls.push(filter);
      const records = filter === 'cites:W100' ? [forwardRec] : filter === 'ids.openalex:W900' ? [backwardRec] : [];
      return { family: 'openalex', query: filter, httpStatus: 200, records, latencyMs: 1 };
    };
    ...
    expect(out.kind).toBe('done');
    if (out.kind === 'done') expect(out.summary).toContain('citation chase');

    // both chase directions executed with the documented filter syntax
    expect(filteredCalls).toContain('cites:W100');
    expect(filteredCalls).toContain('ids.openalex:W900');

    // corpus snapshot: purpose citation_chase logged; chase counter on the fusion record
    const corpus = defined(env.store.listObjects('corpus_snapshot', env.run.id)[0], 'corpus');
    expect(corpus.queries.some((q) => q.purpose === 'citation_chase')).toBe(true);
    expect(corpus.fusion?.citationChaseSearches).toBe(2);

    // chase documents EARNED pool seats through the same dedup+selection machinery
    const docs = env.store.listObjects('source_document', env.run.id);
    expect(docs.some((d) => d.identifiers.some((i) => i.value === '10.1000/chase.forward'))).toBe(true);
    expect(docs.some((d) => d.identifiers.some((i) => i.value === '10.1000/chase.backward'))).toBe(true);

    // every chase search is receipted like any real search
    const chaseReceipts = env.store
      .listObjects('receipt', env.run.id)
      .filter((r) => r.kind === 'source_retrieval' && r.sourceRetrieval !== undefined
        && (r.sourceRetrieval.query === 'cites:W100' || r.sourceRetrieval.query === 'ids.openalex:W900'));
    expect(chaseReceipts).toHaveLength(2);
    for (const r of chaseReceipts) {
      expect(r.sourceRetrieval?.resultCount).toBe(1);
      expect(r.sourceRetrieval?.contentHashes).toHaveLength(1);
    }
  });
