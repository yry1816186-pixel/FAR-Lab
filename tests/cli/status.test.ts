...
test('collectStatusDump: phase A cheap 字段从仓库实测', () => {
  ...
  // migrationCount：0001-0018 共 18 个（status_dump 从 schema/migrations/*.sql readdir 实测·非硬编码；
  // 0018 = FUSION-OS-6 evidence provenance DB trigger·闭合 0017 跨列不变式的纵深防御第二层）。
  assert.strictEqual(dump.migrationCount, 18);
