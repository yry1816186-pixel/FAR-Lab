...
//
// Authority: PROJECT_PLAN/DEPTH_LEDGER.md §C FUSION-OS-7 +
//            PROJECT_PLAN/FUSION_OPEN_SCIENCE_DESIGN.md §4 FUSION-OS-7（per-cell 三元组范式）。
...
function findPythonCommand(): string | null {
  for (const command of ['python3', 'python']) {
    ...
    if (r.error === undefined && r.status === 0) return command;
  }
  return null;
}
