...
test('hero_a_pipeline: real src/statistics drives R7 CONFIRMED -> ASK-9 INCONCLUSIVE seal (P1-5 Phase 2)', () => {
  ...
  try {
    ...
    // ── FUSION-OS-1 生产路径反剧场物证：buildHeroAChain 真跑 runAntiTheaterLint 产 report ──
    // hero_a 是 full-scope caller（R4 不 shadow·anti-theater :373 可达），report 经 fecAppendClaim 真嗂 kernel。
    assert.ok(chain.antiTheaterReport !== undefined, 'buildHeroAChain must run runAntiTheaterLint and produce antiTheaterReport (FUSION-OS-1 full-scope production caller)');
    assert.equal(Array.isArray(chain.antiTheaterReport.findings), true, 'antiTheaterReport.findings must be an array (real lint output)');
    assert.equal(chain.antiTheaterReport.llmOverrideRejected, true, 'llmOverrideRejected=true proves report from real runAntiTheaterLint (deterministic kernel, F3)');
    assert.equal(typeof chain.antiTheaterReport.antiTheaterScore, 'number', 'antiTheaterScore must be a real computed number');
  } finally {
    db.close();
  }
});
