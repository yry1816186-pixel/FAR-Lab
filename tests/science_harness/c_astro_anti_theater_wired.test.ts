...
//
// FUSION-OS-1 anti-theater 通道物证：real runAntiTheaterLint seed-cherry finding → fecAppendClaim → real kernel UNTESTED（证通道活·非生产 caller E2E）。
//
...
// 本测试不调 buildCAstroChain（生产 caller 接线由 env-gated c_astro_pipeline.test.ts 实跑见证：report 生产）；此处经 buildCAstroAntiTheaterInput 构造 lint 输入真跑 detector，证 anti-theater 通道活——非生产路径 E2E，亦非 caller 接线 load-bearing 物证。通道活仅证于人工 full-scope 构造（scopeNarrowerThanClaim=false → R4 不 fire → 触达 :373）；c_astro 真实生产路径 cached_fixture → scope-narrowed → R4（verdict_kernel_v2.ts:345）先于 anti-theater（:373）fire → ANTI_THEATER_FAIL 结构性不可达。
//
...
test('real_seed_cherry_finding_drives_kernel_UNTESTED: real runAntiTheaterLint seed-cherry -> fecAppendClaim -> kernel UNTESTED (anti-theater channel live, FUSION-OS-1)', () => {
