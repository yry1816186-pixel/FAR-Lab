 ...
 *
 * 权威：archived-plan §2.1（检查表）+ §2.2（伪代码）+ §2.3（降级）+ §5.2（verdict_mapping）。
...
test('#5b EVIDENCE_REQUIREMENT_MISSING: requiredEvidence 空 → UNTESTED（BUG-1/D-01·compiler.ts:310 进 hash 却曾无编译期检查）', () => {
  const result = compileFec({ fec: makeValidFec({ requiredEvidence: [] }) });
  assertFail(result);
  assert.ok(hasError(result, 'EVIDENCE_REQUIREMENT_MISSING'));
});
