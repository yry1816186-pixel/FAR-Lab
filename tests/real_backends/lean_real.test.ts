...
test('Lean real backend verifies simple theorem via rfl', async (t) => {
  ...
  const probe = spawnSync(leanCommand, ['--version'], { encoding: 'utf8' });
  if (probe.error !== undefined || probe.status !== 0) {
    t.skip(`lean --version failed for ${leanCommand}: ${(probe.stderr ?? probe.error?.message ?? '').trim()}`);
    return;
  }
  ...
  try {
    const result = spawnSync(leanCommand, [leanPath], { encoding: 'utf8' });
