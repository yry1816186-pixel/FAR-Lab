...
test('LP-2: --write rewrites drifted AGENTS.md claim to actual (auto SSOT sync)', () => {
  // 漂移场景：fixture 声称 1000 而实测 2703——--write 须把 AGENTS.md 回写到实测值。
  const spec = [
    'ℹ tests 2703',
    'ℹ pass 2697',
    'ℹ fail 0',
  ...
  try {
    ...
    assert.equal(claimed.tests, 2703, 'AGENTS.md must be rewritten to actual tests count');
    assert.equal(claimed.pass, 2697, 'AGENTS.md pass must be rewritten to actual');
