...
/** #5 EVIDENCE_REQUIREMENT_MISSING：dataset/workflow requirement 各 ≥1（03 §2.1 #5）。 */
function checkEvidenceRequirements(fec: FecContractV2, errors: CompileError[]): void {
  if (fec.datasetRequirements.length === 0 || fec.workflowRequirements.length === 0) {
    errors.push({
      code: 'EVIDENCE_REQUIREMENT_MISSING',
      severity: mapCompileErrorToSeverity('EVIDENCE_REQUIREMENT_MISSING'),
      message: 'datasetRequirements 与 workflowRequirements 须各 ≥1',
