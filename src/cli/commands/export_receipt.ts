...
export function runExportReceipt(options: ExportReceiptOptions): number {
  ...
  if (sourceCount !== 1) {
    process.stderr.write('far export receipt: 须且只能提供 --envelope 或 --bundle\n');
    return 2;
  }
...
function buildReceiptFromEnvelopeFile(
  ...
  if (!loaded.ok) {
    return { ok: false, exitKind: '载入 envelope 失败', exitCode: 1, error: loaded.error };
  }
  const receipt = buildTrustReceiptFromEnvelope(loaded.envelope, envelopePath, generatedAt);
  if (!receipt.ok) {
    return { ok: false, exitKind: 'envelope 验证失败', exitCode: 7, error: receipt.error };
  }
  return receipt;
}
...
function buildReceiptFromBundle(
  ...
  if (!existsSync(bundlePath)) {
    return { ok: false, exitKind: 'bundle 不存在', exitCode: 1, error: bundlePath };
  }
  const receipt = buildTrustReceiptFromBundle(bundlePath, generatedAt);
  if (!receipt.ok) {
    return { ok: false, exitKind: 'bundle 验证失败', exitCode: 7, error: receipt.error };
  }
  return receipt;
}
