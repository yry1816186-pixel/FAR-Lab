    ...
    if (process.env.FAR_DASHSCOPE_API_KEY === undefined || process.env.FAR_DASHSCOPE_API_KEY === '') {
      process.stderr.write(
        `far ask: profile "${args.profile}" needs real LLM credentials.\n` +
          '  set FAR_DASHSCOPE_API_KEY=sk-xxx and retry (qwen_vl_adapter real HTTP).\n' +
          '  default offline_replay needs no credentials (fixture replay).\n',
      );
      return 2;
    }
    process.stderr.write(
      `far ask: profile "${args.profile}" + a real-credential path needs a reproHashProvider (plugs into 03 calc_bridge).\n` +
        '  the CLI currently supports offline_replay only (zero-key · fixture). For the real-profile path see docs/quickstart.md.\n',
    );
    return 2;
  }
