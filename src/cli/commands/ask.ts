...
export function parseAskArgs(argv: readonly string[]): AskArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--mode') {
      const v = argv[++i];
      if (v !== 'full' && v !== 'quick') {
        throw new Error(`far ask: --mode 须为 full|quick（实际: ${v ?? '<missing>'}）`);
      }
      mode = v;
      continue;
    }
    ...
    if (a.startsWith('--')) {
      throw new Error(`far ask: 未知参数 "${a}"`);
    }
    if (question === '') {
      question = a;
    } else {
...
function renderHuman(args: AskArgs, render: AskRender): void {
  ...
  if (render.verdict !== null) {
    ...
    lines.push('  verdict  : <未到达裁决阶段>');
  }
  lines.push(`  chain    : ${render.chainHeadHash ?? '<空链>'}`);
  ...
  lines.push('');
  lines.push('  红线：裁决由 R0-R9 确定性内核给出（LLM 非裁决者）。offline verdict 仅供流程展示。');
  lines.push('');
...
export async function runAsk(argv: readonly string[]): Promise<number> {
  ...
  if (args.question.trim().length === 0) {
    process.stderr.write(
      'far ask: 缺少 question。\n  用法: far ask "<问题>" [--mode full|quick] [--json] [--export <dir>] [--profile offline_replay]\n',
    );
    return 2;
  }

  if (args.profile !== 'offline_replay') {
    if (process.env.FAR_DASHSCOPE_API_KEY === undefined || process.env.FAR_DASHSCOPE_API_KEY === '') {
      process.stderr.write(
        `far ask: profile "${args.profile}" 需真实 LLM 凭据。\n` +
          '  设置 FAR_DASHSCOPE_API_KEY=sk-xxx 后重试（qwen_vl_adapter 真实 HTTP）。\n' +
          '  默认 offline_replay 无需凭据（fixture 回放）。\n',
      );
      return 2;
    }
    process.stderr.write(
  ...
  if (exportDir !== null && result !== null) {
    ...
    if (exp !== 0) {
      process.stderr.write(`far ask: --export 失败 (exit ${exp})\n`);
      return exp;
    }
    if (!args.json) {
      process.stdout.write(
        `  导出     : ${exportDir}  (far verify --bundle ${exportDir} 可重算验证)\n\n`,
      );
    }
  }
