...
export function parseOptions(
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg.startsWith('--')) {
      ...
      if (opt === undefined) {
        errors.push(`${commandPrefix}: unknown argument '${arg}'`);
        continue;
      }
      ...
      if (inlineValue !== undefined) {
        ...
        const next = args[i + 1];
        if (next === undefined) {
          errors.push(`${commandPrefix}: ${opt.name} requires a value (${opt.description})`);
          continue;
        }
        value = next;
        i += 1;
      ...
      if (opt.type === 'enum' && opt.enumValues !== undefined) {
        if (!opt.enumValues.includes(value)) {
          errors.push(
            `${commandPrefix}: ${opt.name} expects one of ${opt.enumValues.join('|')} (got: ${value})`,
          );
          continue;
        }
      }
  ...
  for (const opt of schema) {
    if (opt.required === true && values[opt.name] === undefined) {
      ...
      errors.push(`${commandPrefix}: missing required option ${opt.name} <${placeholder}>`);
    }
  }
