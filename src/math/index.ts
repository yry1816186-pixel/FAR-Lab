...
// Core types & constants (§1-§4)
export * from './math_claim.ts';

// Errors (§4.5)
export * from './errors.ts';

// Autoformalizer (§6) — model-neutral core + competition adapter
export * from './autoformalizer.ts';
export * from './competition_math_adapter.ts';

// Backends (§3-§4) — L1 CAS / L2 SMT / L3 Formal / L4 Numeric
...
// Verifier router (§4) — explicit exports to avoid BACKEND_KINDS clash
export { MathVerifierOptions, MathVerifier, createDefaultMathVerifier } from './math_verifier.ts';

// Evidence sink (§5) — persistence + evidence_log append
export * from './evidence_sink.ts';

// Premise search (§7) — LeanDojo ReProver style
export * from './premise_search.ts';

// Math gate (§8) — extends 07 falsifiability
export * from './math_gate.ts';

// Honesty wall (§9) — verification boundary rendering
