...
export interface LoopRunnerArgs {
  ...
  /**
   * R3（V2 信封生产者）：裁决计算观测回调（透传 runAgentLoop.onComputation）。
   * 缺省 → 零行为变化（字节等同基线·回归兼容）。
   */
  readonly onComputation?: (
    computation: import('../../agent_loop/verdict_stage.ts').VerdictComputation,
  ) => void;
  /**
