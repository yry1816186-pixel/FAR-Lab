 ...
 *
 * 这是 **ConfoundingGate 的粗粒度分类轴**（为混淆分析设计：existence/quantitative/causal），
 * 与 03 SciIR 的细粒度 9 值科学分类枚举（causal/correlational/mechanistic/predictive/existence/
 * optimization/methodological/measurement/reproducibility）是**不同轴**，非冲突。'quantitative' 此处是
 * 混淆分析语义（含量化测量 vs 纯存在性 vs 因果），映射 SciIR 的 {measurement, correlational, predictive,
 * optimization}。两者同名 claimType 是历史命名，本类型为 ConfoundingGate 专用粗粒度子集。'causal' 触发 F6。
 */
