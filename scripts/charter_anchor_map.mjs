#!/usr/bin/env node
/**
 * charter_anchor_map.mjs — ULTIMATE_EXECUTION_PRIME.md 锚点导航系统
 *
 * 扫描 docs/charter/ULTIMATE_EXECUTION_PRIME.md 的 ^#{1,3} 标题(围栏感知,跳过代码块内的伪标题),
 * 按「编」分组成锚点并生成 agent/contracts/charter-anchor-map.yaml。
 * 宪章变更后只需重跑本脚本;下游引用锚点而非硬编码行号。
 *
 * 用法:
 *   node scripts/charter_anchor_map.mjs            重新生成 yaml
 *   node scripts/charter_anchor_map.mjs --check    校验 yaml 与宪章的 md5 一致性(不一致 exit 1)
...
const MD_PATH = path.join(ROOT, 'docs', 'charter', 'ULTIMATE_EXECUTION_PRIME.md');
...
function extractHeadings(lines) {
  ...
  if (inFence) {
    console.error('ERROR: ULTIMATE_EXECUTION_PRIME.md code fences are unbalanced (file ends inside a fence).');
    process.exit(2);
  }
  return headings;
}
...
function toYaml(anchors, md5, totalLines) {
  ...
  out.push('# ULTIMATE_EXECUTION_PRIME.md 变更后重跑: node scripts/charter_anchor_map.mjs');
  out.push(`generated_at: ${q(new Date().toISOString())}`);
  out.push(`source: ${q('docs/charter/ULTIMATE_EXECUTION_PRIME.md')}`);
...
function cmdCheck() {
  ...
  if (m[1] !== md5) {
    console.error(`ERROR: ULTIMATE_EXECUTION_PRIME.md changed since map generation (yaml=${m[1]}, actual=${md5}).`);
    console.error('Re-run: node scripts/charter_anchor_map.mjs');
    process.exit(1);
  }
  console.log(`OK: charter-anchor-map.yaml is in sync with ULTIMATE_EXECUTION_PRIME.md (md5=${md5}).`);
}
