...
//
// 设计目的（为什么-不是什么）：
//   现有 6 个 scan 脚本全扫「禁用 token 出现」（:any / @ts-ignore / extra_body / secret），
...
function verifyDepthLedger() {
  ...
  if (!ledger.exists) {
    return { passed: false, reason: 'DEPTH_LEDGER.md 不存在——本地开发仓库结构破坏（须有 FAR_LAB_MASTER_PLAN/DEPTH_LEDGER.md 供 depth_gate 校验）' };
  }
