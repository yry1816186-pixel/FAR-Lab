...
- 检索透明面板（D-060 切片4）：证据 tab 顶部展示实际执行的查询计划（每条带 purpose 徽章：发现/支持/**反证**·琥珀）+ 各源命中数（与 source_retrieval 回执按查询文本 join）+ 融合统计行（池/重排/反证保留席位/恢复变体次数，全部真实计算值）+ 源失败如实 callout；retrieve 未完成时显示阶段感知空态。数据源=新端点 `GET /runs/:id/corpus`。
